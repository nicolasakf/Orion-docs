...
Orion ships with **built-in skills** — instruction packages the assistant can load automatically or on demand with **`/skill-name`**. They teach the agent Orion-specific workflows: notebook UI, App View layouts, settings, metadata, sub-agents, custom skills, workspace rules, Research-mode exploratory data analysis, and chat history search.
